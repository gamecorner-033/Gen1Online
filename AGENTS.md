...
- **Server logging** (`src/gen1online/logging_utils.py`, stdlib `logging` → stderr, visible via `docker compose logs -f gts-server`): INFO = player join/leave, challenges, deposits/trades/withdraws/claims, profile updates, trade receipts, purge notices, admin actions. `sync_pos` ticks, `poll_battle_msgs` and `send_battle_msg` stay silent at INFO (10–20 Hz noise); they log at DEBUG (`LOG_LEVEL=DEBUG`). Unhandled handler exceptions log a traceback and return a JSON 500. `docker-compose.yml` passes `LOG_LEVEL: ${LOG_LEVEL:-INFO}` through.
- **Admin API** (`src/gen1online/admin.py`, gated by `Authorization: Bearer <token>`, token from `ADMIN_TOKEN` env — empty disables it with 403): GET `/gts/admin/players`, `/gts/admin/bans`, `/gts/admin/announcement`; POST `/gts/admin` with `action` one of `kick`/`ban`/`unban`/`remove_listing`/`announce`/`clear_announcement`. Bans are transient in-memory (defaults: kick 300s, ban 86400s); a banned trainer's `sync_pos` returns 403. Announcements are delivered in the `sync_pos` response (client ignores unknown keys).
- **Stats/metrics/healthz** (`src/gen1online/metrics.py`, `stats.py`): public `GET /gts/stats` (online/peak, players-by-map, listings, pending claims, today + 7d daily counters) and Prometheus `GET /metrics`. Daily counters (`joins`/`deposits`/`trades`/`claims`/`withdrawals`) live in the Postgres `daily_counts` table, bumped inside the `persist_*` transactions. `GET /healthz` pings Postgres (`connect_timeout=2`), 503 when down; `docker-compose.yml` wires it as the gts-server healthcheck. F-string logging is required.
